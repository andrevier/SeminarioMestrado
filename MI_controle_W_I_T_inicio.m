% Inicialização do modelo de maquina de indução com controle de velocidade, torque e corrente para o SVPWM.
...
disp('Malha de controle de velocidade')
...
disp('Malha de controle de corrente antes do SVPWM')
...
disp('Malha de controle de torque.')
% Controle de Torque
Wcii = 100*Wc
MFii = 60*pi/180
tanMF = tan(MFii - pi)

% Kiii = J*Wcii^2/sqrt(1+tanMF^2)
% Kpii = tanMF*J*Wcii/sqrt(1+tanMF^2)
Kiii = Wcii*(Wcii*sigma*Ls + Rs*tanMF)/sqrt(tanMF^2 + 1)
Kpii = (Kiii/Wcii)*(Wcii*sigma*Ls*tanMF - Rs)/(Wcii*sigma*Ls + Rs*tanMF)
% Conferir a função de transferência:
s = j*Wcii;
% GL_ii = (Kpii*s + Kiii)/(J*s^2)
% GLabs_ii = abs(GL_ii)
...
sim('MI_controle_W_T_I',2.0)

%gráficos
figure(1)
subplot(211)
plot(torque_ts)
title('Torque da carga x Torque eletrico.')

subplot(212)
plot(Wmech_ts)
title('Velocidade mecanica de referencia x medida.')